...
The release workflow is different: release builds run only for explicit `vX.Y.Z`
tag pushes or manual `workflow_dispatch` runs. GitHub does not apply path
filters to tag pushes, and a release tag is an explicit publishing action, so
release packaging should not be skipped by source-path filters.
