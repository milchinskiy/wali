...
pub enum Error {
    ...
    ModuleRequirement { id: String, message: String },
...
impl std::fmt::Display for Error {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::ModuleSource(e) => write!(f, "Module source error: {e}"),
            Self::ModuleSchema { path, message } => write!(f, "Invalid module input data: {path}: {message}"),
            Self::RequirementCheck(e) => write!(f, "Requirement check failed: {e}"),
            Self::ModuleRequirement { id, message } => write!(f, "{id} requirements: {message}"),
...
impl std::error::Error for Error {
    fn source(&self) -> Option<&(dyn std::error::Error + 'static)> {
        match self {
            ...
            Self::ModuleSource(_) => None,
            Self::ModuleSchema { .. } => None,
            Self::RequirementCheck(_) => None,
...
impl From<Error> for ap::Error {
    fn from(value: Error) -> Self {
        let code = match value {
            ...
            Error::ModuleSource(..) => 24,
            Error::ModuleSchema { .. } => 26,
            Error::RequirementCheck(..) => 27,
            Error::ModuleRequirement { .. } => 28,
